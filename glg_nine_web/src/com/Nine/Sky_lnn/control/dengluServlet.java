...
public class dengluServlet extends HttpServlet {
	...
	private void leave(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		request.getSession().removeAttribute("glyadmin");
		request.getSession().removeAttribute("glyxx");
		//response.sendRedirect(request.getContextPath() + "/index.jsp");
		request.getRequestDispatcher("/XxglServlet?m=listAll&flag=first").forward(request,response);
	}
	...
	protected void login(HttpServletRequest request, HttpServletResponse response)
		...
		if (a && b && dlxxb.getSfbz().equals("1")) {
			...
			response.sendRedirect(request.getContextPath() + "/login.jsp?n=0&zh=" + zh);
		}
